...
class GSNNRunner(_base.BaseRunner):
    ...
    def _init_engines(self) -> typ.Tuple[ie.Engine, ie.Engine]:
        ...
        trainer = pe.create_gsnn_trainer(self.model,
                                         self.optimizer,
                                         self.criterion,
                                         trainer_metrics,
                                         self.device)
        evaluator = pe.create_gsnn_evaluator(self.model,
                                             evaluator_metrics,
                                             self.device,
