...
class ClassEvaluator(_base.BaseEvaluator):
    ...
    def _init_evaluators(self) -> typ.Tuple[ie.Engine, ie.Engine, ie.Engine]:
        train_evaluator = pe.create_cls_evaluator(self.model, self.opts.evaluator.metrics, self.device, True)
        dev_evaluator = pe.create_cls_evaluator(self.model, self.opts.evaluator.metrics, self.device, True)
        test_evaluator = pe.create_cls_evaluator(self.model, self.opts.evaluator.metrics, self.device, True)
