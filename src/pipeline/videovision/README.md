...
Several transforms are then provided in [video_transforms](transforms/video_transforms.py).
...
This can be produced by the [volume_transform](transforms/volume_transforms.py).**ClipToTensor** class, which produces a video volume in format (n_channels, n_images, height, width) where n_channels = 3 in case of images.
...
[video_transforms.py](transforms/video_transforms.py)
- [x] Compose
- [x] Resize
- [x] CenterCrop
- [x] RandomCrop
...
[volume_transforms.py](transforms/volume_transforms.py)
- [x] ClipToTensor
