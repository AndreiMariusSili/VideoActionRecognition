...
from models import criterion, metrics as custom_metrics, vae_i3d
...
trainer_opts = model_opts.TrainerOptions(
    ...
    criterion_opts=model_opts.VAECriterionOptions(
        mse_factor=1.0,
        ce_factor=64.0,
        kld_factor=1.0
    ),
    metrics={
        'acc@1': metrics.Accuracy(output_transform=lambda x: (x[1].squeeze(dim=1), x[6])),
        'acc@2': metrics.TopKCategoricalAccuracy(k=2, output_transform=lambda x: (x[1].squeeze(dim=1), x[6])),
        'mse_loss': mse_loss,
        'ce_loss': ce_loss,
        'kld_loss': kld_loss,
...
# RUN
########################################################################################################################
vaei3d_smth_100 = model_opts.RunOptions(
    name='vaei3d_smth_100',
    mode='vae',
    resume=False,
    log_interval=10,
    patience=10,
