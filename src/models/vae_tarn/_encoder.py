...
from models.vae_tarn import _helpers as hp
...
class VariationalSpatialResNetEncoder(nn.Module):
    ...
    def _make_layer(self, out_planes: int, blocks: int, stride=1):
        downsample = None
        if stride != 1 or self.in_planes != out_planes * SpatialResidualBlock.expansion:
            downsample = nn.Sequential(
                hp.conv1x1(self.in_planes, out_planes * SpatialResidualBlock.expansion, stride),
                nn.BatchNorm2d(out_planes * SpatialResidualBlock.expansion),
            )
        layers = [
            SpatialResidualBlock(self.in_planes, out_planes, stride, downsample)
        ]
        self.in_planes = out_planes * SpatialResidualBlock.expansion
        for _ in range(1, blocks):
            layers.append(SpatialResidualBlock(self.in_planes, out_planes, 1))
...
class VariationalTemporalResidualBlock(nn.Module):
    ...
    def __init__(self, in_planes: int, out_planes: int, stride: int = 1):
        ...
        self.mean = hp.conv1x1(out_planes, out_planes)
        self.var = nn.Sequential(
            hp.conv1x1(out_planes, out_planes),
            nn.Softplus()
        )
