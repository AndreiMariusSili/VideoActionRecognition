...
GSNN_FORWARD = tp.Tuple[th.Tensor, th.Tensor, th.Tensor, th.Tensor, th.Tensor, tp.Optional[th.Tensor]]


class GSNNI3D(nn.Module):
    def __init__(self, time_steps: int, latent_planes: int, dropout_prob: float, num_classes: int, vote_type: str):
        super(GSNNI3D, self).__init__()
    ...
    def _forward(self, _in: th.Tensor, num_samples: int) -> GSNN_FORWARD:

        temporal_latents, _mean, _var = self.encoder(_in, num_samples)
        _pred, _class_latent = self.classifier(temporal_latents)

        return _pred, temporal_latents, _class_latent, _mean, _var, None
    ...
    def _inference(self, _in: th.Tensor, num_samples: int) -> GSNN_FORWARD:
        ...
        return _pred, _temporal_latents, _class_latent, _mean, _var, _vote
