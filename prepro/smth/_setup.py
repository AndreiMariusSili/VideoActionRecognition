...
FULL_WEBM_DIR = ghp.change_setting(ct.SMTH_WEBM_DIR, 'dummy', 'full')
FULL_JPEG_DIR = ghp.change_setting(ct.SMTH_JPEG_DIR, 'dummy', 'full')
FULL_ARCHIVE_DIR = ghp.change_setting(ct.SMTH_ARCHIVE_DIR, 'dummy', 'full')
FULL_ROOT_DIR = ghp.change_setting(ct.SMTH_ROOT_DIR, 'dummy', 'full')
FULL_META_TRAIN_1 = ghp.change_setting(ct.SMTH_META_TRAIN_1, 'dummy', 'full')
FULL_META_VALID_1 = ghp.change_setting(ct.SMTH_META_VALID_1, 'dummy', 'full')
FULL_META_TEST_1 = ghp.change_setting(ct.SMTH_META_TEST_1, 'dummy', 'full')
FULL_LID2GID_1 = ghp.change_setting(ct.SMTH_LID2GID_1, 'dummy', 'full')


def __video_path(row: pd.Series) -> str:
    """Create path to video relative to data root dir."""
    return (FULL_WEBM_DIR / row['template'] / f'{row.id}.webm').relative_to(FULL_ROOT_DIR).as_posix()


def __image_path(row: pd.Series) -> str:
    """Create path to image folder relative to data root dir."""
    return (FULL_JPEG_DIR / row['template'] / f'{row.id}').relative_to(FULL_ROOT_DIR).as_posix()


def _make_data_dirs():
    """Create directories."""
    os.makedirs(ghp.change_setting(ct.SMTH_META1_DIR, 'dummy', 'full').as_posix(), exist_ok=True)
    os.makedirs(ghp.change_setting(ct.SMTH_WEBM_DIR, 'dummy', 'full').as_posix(), exist_ok=True)
    os.makedirs(ghp.change_setting(ct.SMTH_JPEG_DIR, 'dummy', 'full').as_posix(), exist_ok=True)
    os.makedirs(ghp.change_setting(ct.SMTH_ARCHIVE_DIR, 'dummy', 'full').as_posix(), exist_ok=True)

    os.makedirs(ghp.change_setting(ct.SMTH_META1_DIR, 'full', 'dummy').as_posix(), exist_ok=True)
    os.makedirs(ghp.change_setting(ct.SMTH_WEBM_DIR, 'full', 'dummy').as_posix(), exist_ok=True)
    os.makedirs(ghp.change_setting(ct.SMTH_JPEG_DIR, 'full', 'dummy').as_posix(), exist_ok=True)


def _create_meta():
    """Create meta files. Groups are read from .txt file and labels with no group are assigned a new singular group."""
    meta_train = pd.read_json((FULL_ROOT_DIR / META_TRAIN_NAME).as_posix(), orient='records')
    meta_valid = pd.read_json((FULL_ROOT_DIR / META_VALID_NAME).as_posix(), orient='records')
    meta_test = pd.read_json((FULL_ROOT_DIR / META_TEST_NAME).as_posix(), orient='records')
    items = [
        ['train', FULL_META_TRAIN_1, meta_train],
        ['validation', FULL_META_VALID_1, meta_valid],
        ['test', FULL_META_TEST_1, meta_test],
    ]

    labels = _create_labels()
    groups = _create_groups()
    lid2gid = _create_lid2gid(groups, labels)
    lid2gid.to_json(FULL_LID2GID_1, orient='index')
...
def _create_labels():
    ...
    with open((FULL_ROOT_DIR / LABELS_NAME).as_posix(), 'r') as file:
...
def _create_groups() -> pd.DataFrame:
    ...
    with open((FULL_ROOT_DIR / GROUPS_NAME).as_posix(), 'r') as file:
        txt = file.read()
...
def _extract():
    ...
    subprocess.call(f'cat 20bn-something-something-v2-?? | tar zx',
                    ...
                    cwd=FULL_ROOT_DIR.as_posix(),
                    shell=True)


def _move_webm():
    """Move webm files to the webm folder."""
    meta_train = ghp.read_meta(FULL_META_TRAIN_1).set_index('id', verify_integrity=True)
    meta_valid = ghp.read_meta(FULL_META_VALID_1).set_index('id', verify_integrity=True)
    meta = pd.concat([meta_train, meta_valid], verify_integrity=True)

    webms = ghp.change_setting(FULL_ROOT_DIR, 'dummy', 'full') / '20bn-something-something-v2'
    for webm in webms.glob('*.webm'):
        ...
        file_path = ghp.change_setting(FULL_WEBM_DIR, 'dummy', 'full') / label / file_name
...
def _cleanup():
    ...
    for path in FULL_ROOT_DIR.glob('20bn-something-something-v2-??'):
        archive_path = FULL_ARCHIVE_DIR / path.name
        path.replace(archive_path.as_posix())

    for file in [META_TRAIN_NAME, META_VALID_NAME, META_TEST_NAME, LABELS_NAME, GROUPS_NAME]:
        root_path = (FULL_ROOT_DIR / file)
        archive_path = FULL_ARCHIVE_DIR / file
